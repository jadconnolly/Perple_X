...
Generally you will not be in your working directory if you start Julia. An easy way to change your directory is by using the built-in terminal, which you can reach by typing `;` in the REPL:
```julia
julia> ;
shell> 
```
...
Yet, in addition to the binaries themselves, you will also have to tell your system where some required dynamic libraries (such as libgfortran) are located.
```julia
