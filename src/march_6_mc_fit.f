...
c                                 *_central.pts file
            if (invrnd.eq.1) write (n0,1025) 3, (x(j),j=1,nparm), objf
...
c                                write phase name for molar composition output
         ...
         do i = 1, kbulk
c                                 absent is a flag to check that every component
c                                 specified for the system that isn't an unmeasured
c                                 component has been used
            if (comp(i).ne.0d0) absent(i) = .false.

            if (absent(i).and.redox.ne.i) then
c                                 if the component is absent and it's NOT
c                                 an umeasured component then if the component
c                                 IS present
...
c-----------------------------------------------------------------------
     ...
     *        nmiss, jct(k5), jsol(k5,k5), jj, jbest
...
c                                 output any model that fits within error to *.fit
         write (n12,1140) invrnd, (x(k), k = 1, nparm), 
     *                    (cblk(k), k = 1, kbulk),
