...
1000  format(132a1)
2000  format(a8,4(i2),' | H = ',g14.7,2x,a)
...
c                                            a gas
       ...
       ilam = lam * 10
c                                            ds6.36 changed the codes for special cases:
c                                            gas => lam = 6
c                                            melt => lam = 7
c                                            aq => lam = 8

      if(lam.gt.0.and.lam.le.2) then
...
c           if (l6.lt.0d0) l6 = (-l6*l5 + 1d0)/(l5 + 1d0)
c                             who knows when/where that came from, my 
c                             working hypothesis is that when l6 < 0
c                             it is the factor on n to give the fake multiplicity 
c                             on the  n-fold site. this works for sp/herc and 
c                             currently (ds6.36/7.2.0) there are no other examples.
            if (l6.lt.0d0) write (*,*) name,' has negative fac =',l6
...
c                                      2 - ideal gas
      if (aq) then
       ...
       write (10,2000) name,itype,0,ilam,0,g, lname
c                                    convert hp H to G
